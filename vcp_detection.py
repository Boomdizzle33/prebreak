...
def is_valid_vcp(ticker):
    ...
    try:
        ...
        # ✅ Fix: Ensure all values are correctly extracted
        vcp_score = (
            (df['ATR_Contraction'].iloc[-1] * VCP_WEIGHTS['ATR_Contraction']) +
            (df['Volume_Contraction'] * VCP_WEIGHTS['Volume_Contraction']) +
            (int(pullback_contraction) * VCP_WEIGHTS['Pullback_Contraction']) +
            (df['Pivot_Level'] * VCP_WEIGHTS['Pivot_Level']) +
            (int(in_trend) * VCP_WEIGHTS['SMA_Trend']) +
            (int(near_high) * VCP_WEIGHTS['52_Week_High']) +
            (int(volume_expansion) * VCP_WEIGHTS['Volume_Expansion']) +
            (int(strong_closing_range) * VCP_WEIGHTS['Closing_Strength'])
        )
